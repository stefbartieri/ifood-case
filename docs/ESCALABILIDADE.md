...
| **Evolução de schema** | A origem adiciona, renomeia ou muda o tipo de uma coluna | Manter o mapeamento canônico falhando em coluna desconhecida e classificar a mudança pela política do [contrato de dados](DATA_CONTRACT.md), que separa o que quebra compatibilidade do que não quebra | Aceitar coluna nova em silêncio é como o schema de um lake apodrece. Já aconteceu nesta base entre 2023-01 e 2023-02. O contrato é o que transforma essa decisão em regra combinada com quem consome, em vez de escolha caso a caso |
...
| **Particionamento e compactação** | Tabela passa de **1 TB** (abaixo disso a recomendação é não particionar) e cada partição sustenta pelo menos **1 GB** | Entre 1 TB e 100 TB, **liquid clustering** em vez de partição estática; acima de 1 TB, `OPTIMIZE` agendado — se a otimização preditiva não estiver ativa | A camada de consumo tem 152 MiB e não é particionada, exatamente por isso. A camada conformada é particionada com partições de ~27 MB — **bem abaixo** do mínimo de 1 GB —, e isso é deliberado: ali a partição serve de unidade de reprocessamento, não de otimização de leitura. Em produção esses dois papéis se separam. Vale notar que as tabelas gerenciadas aqui já têm **otimização preditiva ativa por herança do metastore**, o que dispensa `OPTIMIZE` manual nesta faixa |
